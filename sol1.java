...
public class Solution{
    ...
    public static void main(String[] args){
        ...
        sc.close();
        Associate[] result = associateForGivenTechnology(associates, searchTechnology);
        for(int i = 0;i<result.length;i++)
           System.out.println(result[i].getId());
    }
    
    
    public static Associate[] associateForGivenTechnology(Associate[] associates, String searchTechnology){
        ...
        for(int i = 0;i<associates.length;i++){
            if(associates[i].getTechnology().equals(searchTechnology) && associates[i].getExperienceInYears()%5==0){
                ...
                result[result.length-1]= associates[i];
            }
        }
        return result;
    }
